...
function ThemeManager:AddTheme(name, themeTable)
    ...
    -- Fill gaps from the built-in theme. A partial table would otherwise leave
    -- the library reading nil colours for whichever keys were omitted.
    local merged = {}
    for k, v in self.Themes.Midnight do merged[k] = v end
    for k, v in themeTable do merged[k] = v end
    merged.Name = name
...
function ThemeManager:LoadSaved()
    pcall(function()
        if isfile and not isfile(THEME_FILE) then return end
        -- a writefile round-trip can leave trailing whitespace, which would
        -- never match a theme key
        local saved = readfile(THEME_FILE):match("^%s*(.-)%s*$")
