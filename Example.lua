-- EZ UI Library - Full Example
-- github.com/Beastaive22/EZ-Hub
...
-- restore whichever theme was picked last run (before the window is built,
-- so it opens in the right colours instead of flashing the default first)
...
utilSection:AddButton({
    Text = "Server Hop",
    Callback = function()
        EZ:Notify({Title="Server Hop", Content="Finding server...", Duration=3, Type="info"})
    end
})
...
utilSection:AddParagraph({
    Title = "Info",
    Content = "EZ UI Library by Aiven. Open source on GitHub."
})
...
themeSection:AddDropdown("Theme", {
    ...
    Callback = function(v)
        ...
        EZ:Notify({Title="Theme", Content="Switched to " .. v, Duration=2, Type="success"})
    end
})
...
configSection:AddButton({
    Text = "Save Config",
    Callback = function()
        local name = EZ.Flags["ConfigName"] or "default"
        local ok, err = SaveManager:Save(name)
        EZ:Notify({Title="Config", Content=ok and ("Saved: "..name) or ("Error: "..tostring(err)), Duration=3, Type=ok and "success" or "error"})
    end
})
configSection:AddButton({
    Text = "Load Config",
    Callback = function()
        ...
        EZ:Notify({Title="Config", Content=ok and ("Loaded: "..name) or ("Error: "..tostring(err)), Duration=3, Type=ok and "success" or "error"})
    end
})
...
shareSection:AddButton({
    ...
    Callback = function()
        ...
        if s then
            ...
            EZ:Notify({Title="Export", Content="Copied " .. #s .. " bytes to clipboard", Duration=3, Type="success"})
        end
    end
})
shareSection:AddButton({
    ...
    Callback = function()
        local ok = SaveManager:Import(EZ.Flags.ShareString or "")
        EZ:Notify({Title="Import", Content=ok and "Loaded" or "Failed", Duration=3, Type=ok and "success" or "error"})
    end
})
...
QuickBar:PinButton("Reset", {
    Icon = "rotate-ccw",
    Callback = function()
        EZ:Notify({ Title = "Reset", Content = "All flags reset", Duration = 2, Type = "info" })
        for k, v in EZ.Flags do
            if typeof(v) == "boolean" and v then
                local elem = EZ._elements and EZ._elements[k]
...
-- read the list off the library so the count can never drift from reality
local themeList = {}
for name in EZ.Themes do table.insert(themeList, name) end
table.sort(themeList)

previewSection:AddParagraph({
    Title = "Built-in Pro Themes",
    Content = "Tap any below to apply instantly. EZ ships " .. #themeList .. " curated dev-favorite themes.",
})
...
for _, name in themeList do
    previewSection:AddButton({
        Text = name,
        Callback = function()
            local theme = EZ.Themes and EZ.Themes[name]
            if theme then
                EZ:SetTheme(theme)
                EZ:Notify({ Title = "Theme", Content = name, Duration = 2, Type = "success" })
            end
        end,
    })
end
...
proofSection:AddParagraph({
    ...
    Content = "Quick Bar with icon dock · Config Profiles · Notification History · " .. #themeList .. " pro themes · hardened key system.",
})

proofSection:AddButton({
    Text = "1. Trigger info notification",
    Callback = function() EZ:Notify({ Title="EZ", Content="info ping", Type="info", Duration=3 }) end,
})
proofSection:AddButton({
    Text = "2. Trigger success notification",
    Callback = function() EZ:Notify({ Title="Done", Content="action completed", Type="success", Duration=3 }) end,
})
proofSection:AddButton({
    Text = "3. Trigger warning notification",
    Callback = function() EZ:Notify({ Title="Heads up", Content="check your config", Type="warning", Duration=3 }) end,
})
proofSection:AddButton({
    Text = "4. Trigger error notification",
    Callback = function() EZ:Notify({ Title="Error", Content="something went wrong", Type="error", Duration=3 }) end,
})
proofSection:AddButton({
    ...
    Callback = function()
        for i = 1, 5 do
            task.delay(i * 0.15, function()
                EZ:Notify({ Title = "Burst #" .. i, Content = "stacked", Type = ({"info","success","warning","error","info"})[i], Duration = 4 })
            end)
        end
    end,
})
...
liveLog:Success("EZ 3.2 runtime up")
...
EZ:Notify({
    Title = "EZ v3.2",
    Content = "loaded · " .. #Icons:All() .. " icons · " .. #themeList .. " themes",
    Duration = 4,
    Type = "success",
})
